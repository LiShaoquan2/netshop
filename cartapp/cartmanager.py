...
class SessionCartManager(CartManager):
    ...
    def add(self,goodsid,colorid,sizeid,count,*args,**kwargs):
        ...
        print(key)
        # print(self.session[self.cart_name][key])
        ...
        if key in self.session[self.cart_name]:
            ...
            print("key不在字典")
            # print(self.session[self.cart_name])
    ...
    def update(self,goodsid,colorid,sizeid,step,*args,**kwargs):
        ...
        if key in self.session[self.cart_name]:
            ...
            cartitem.count = str(int(str(cartitem.count))+int(step))
            print(cartitem.count)
            self.session[self.cart_name][key] = jsonpickle.dumps(cartitem)
            print(self.session[self.cart_name][key])
    ...
    def queryAll(self,*args,**kwargs):
        ...
        # print('queryAll方法中')
        # print(self.session[self.cart_name])
        for i in self.session[self.cart_name].values():
            cartItemList.append(jsonpickle.loads(i))
        return cartItemList
...
def getCartManger(request):
    ...
    return SessionCartManager(session=request.session)
